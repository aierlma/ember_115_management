...
# File   : chain.py
# Time   : 2023/12/17 1:51

"""
原作者：T3rry，只是将原作者的代码进行了一些修改，以适应自己的需求
"""
...
class Fake115Client(object):
    def __init__(self, cookie, user_agent):
        ...
        self.g_kts = a2b_hex(
            '')
        self.curve = NIST224p
...
class URLCache:
    """
    URL缓存，用于解决拖动进度条的时候，重复请求的问题
    缓存周期: 1小时
    作者: Jonn
    主页: https://gist.github.com/jonntd
    """

    def __init__(self):
        self.cache = {}
        self.expiry_time = datetime.timedelta(hours=1)

    def _generate_md5(self, key):
        return hashlib.md5(key.encode()).hexdigest()

    def _clean_cache(self):
        current_time = datetime.datetime.now()
        keys_to_delete = [key for key, (_, timestamp) in self.cache.items() if
                          current_time - timestamp >= self.expiry_time]
        for key in keys_to_delete:
            del self.cache[key]

    def get_or_add_to_cache(self, modified_path, user_agent):
        cache_key = self._generate_md5(modified_path)
        current_time = datetime.datetime.now()

        # Clean the cache before checking
        self._clean_cache()

        if cache_key in self.cache:
            cached_download_url, timestamp = self.cache[cache_key]
            if current_time - timestamp < self.expiry_time:
                return cached_download_url

        download_url = get_115url(modified_path, cookie, user_agent)
        if download_url:
            self.cache[cache_key] = (download_url, current_time)
        return download_url
...
def index(path=''):
    ...
    if match and request.args.get('MediaSourceId'):
        ...
        if response.status_code == 200:
            ...
            for media_source in response_data.get('MediaSources', []):
                if media_source.get('Id') == MediaSourceId:
                    for key, value in mount_mapping.items():
                        if Path(media_source['Path']).as_posix().startswith(key):
                            modified_path = os.path.normpath(value + media_source['Path'][len(key.strip('/')) + 1:])
                            modified_path = Path(modified_path).as_posix()
                            cached_download_url = cache.get_or_add_to_cache(modified_path, user_agent)
                            if cached_download_url:
                                return redirect(cached_download_url, code=302)
                            else:
                                return "Failed to retrieve File ID.", 404
